...
class CosFile
{
    ...
    public static function getThird($url)
    {
        ...
        return [
                'srcPath' =>  $tmpthirdPic,
                'bucketName' => HI\Config\QCloud\PUBLIC_BUCKET,
	    ...
	    $result = Execution::autoUnlink(function($unlinkLater) use ($file, $srcPath, $bucketName,
    		...
    		if ($sliceUploadRet['code'] == 0) {
    		    ...
    		    if( $category == self::CATEGORY_SCENE || $category == self::CATEGORY_EVENT) {
    		        switch (HI\Config\QCloud\TENCENT_UPLOAD_SOURCE) {
    		            case 0:
    ...
    public static function face($url)
    {
     ...
     {
         $bucketName = Accessor::either($bucketName, HI\Config\QCloud\BUCKET);
     ...
     {
         return TencentAuth::appSign(time() + HI\Config\QCloud\COS_SIGN_EXPIRE, HI\Config\QCloud\PUBLIC_BUCKET);
     }
     ...
     {
         return TencentAuth::appSign_once($path, HI\Config\QCloud\PUBLIC_BUCKET, time() + HI\Config\QCloud\COS_SIGN_EXPIRE);
     }
     ...
     public static function uploadWeChatImage ($srcPath, $eventId, $momentId)
     {
         ...
         $sliceUploadRet = TencentCosapi::upload_slice($srcPath, HI\Config\QCloud\BUCKET, $tencentFileName);
         if ($sliceUploadRet['code'] == 0) {
     ...
     public static function uploadTo($srcPath, $destPath)
     {
        return TencentCosapi::upload_slice($srcPath, HI\Config\QCloud\PUBLIC_BUCKET, $destPath);
     }
}
