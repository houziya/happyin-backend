...
class TencentCosapi
{
    ...
    public static function generateResUrl($bucketName, $dstPath) {
        return TencentConf::API_COSAPI_END_POINT . TencentConf::APPID . '/' . $bucketName . $dstPath;
    }
        
    public static function sendRequest($req) {
        //var_dump($req);

        $rsp = TencentHttp::send($req);
        $info = TencentHttp::info();
    ...
    public static function upload($srcPath, $bucketName, $dstPath, $bizAttr = null) {
        ...
        $sign = TencentAuth::appSign($expired, $bucketName);
