...
class Accessor implements ArrayAccess, JsonSerializable
{
    private $fallback;
    private $wrapped;
    ...
    public static function copyBetween($from, &$to)
    {
        if (Predicates::isObject($from)) {
            $from = get_object_vars($from);
        }
        foreach ($from as $key => $value) {
            self::doSet($to, $key, $value);
        }
    }
    ...
    public function jsonSerialize() {
        if (is_object($this->wrapped)) {
            $result = get_object_vars($this->wrapped);
        } else if (is_array($this->wrapped)) {
            $result = $this->wrapped;
        } else {
            $result = [];
        }
        if (is_array($this->fallback)) {
            return array_merge($this->fallback, $result);
        } else {
            return $result;
        }
    }
