...
class CPagination{
	...
	public static function showPager($link,&$page_no,$page_size,$row_count, $type = ''){
		$url="";
		$params="";
		if($link != ""){
			$pos = strpos($link,"?");
		...
		if ($page_no > 1){
			$navibar .= "<li><a href=\"$url?page_no=1\">首页</a></li>\n <li><a href=\"$url?page_no=".($page_no-1)." \">上一页</a></li>\n";
		}
		...
		for($i=$start_page;$i<=$end_page;$i++){
			if($i==$page_no){
				...
				$navibar.= "<li><a href=\" $url?page_no=$i \">$i</a></li>";
			}
		}
		
		if ($page_no < $total_page){
			$navibar .= "<li><a href=\"$url?page_no=".($page_no+1)."\">下一页</a></li>\n <li><a href=\"$url?page_no=$total_page\">末页</a></li>\n ";
		}
		if($total_page>0){
