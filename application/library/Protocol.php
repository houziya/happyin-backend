...
class Protocol
{
    ...
    private static function validateIn($value, $valid)
    {
        Preconditions::checkArgument(in_array($value, $valid));
        return $value;
    }

    private static function validateInRange($value, $min, $max)
    {
        Preconditions::checkArgument($value >= $min && $value <= $max);
        return $value;
    }

    public static function requiredIn($name, $valid)
    {
        return self::validateIn(self::required($name), $valid);
    }

    public static function requiredInRange($name, $min, $max)
    {
        return self::validateInRange(self::required($name), $min, $max);
    }
    ...
    public static function optionalInRange($name, $min, $max, $default = NULL)
    {
        return self::validateInRange(self::optional($name, $default), $min, $max);
    }

    public static function optionalIn($name, $valid, $default = NULL)
    {
        return self::validateIn(self::optional($name, $default), $valid);
    }

    public static function requiredIntInRange($name, $min, $max)
    {
        return self::validateInRange(self::requiredInt($name), $min, $max);
    }

    public static function requiredIntIn($name, $valid)
    {
        return self::validateIn(self::requiredInt($name), $valid);
    }
    ...
    public static function optionalIntInRange($name, $min, $max, $default = NULL)
    {
        return self::validateInRange(self::optionalInt($name, $default), $min, $max);
    }

    public static function optionalIntIn($name, $valid, $default = NULL)
    {
        return self::validateIn(self::optionalInt($name, $default), $valid);
    }
    ...
    public static function requiredDoubleInRange($name, $min, $max)
    {
        return self::validateInRange(self::requiredDouble($name), $min, $max);
    }
    ...
    public static function optionalDoubleInRange($name, $min, $max, $default = NULL)
    {
        return self::validateInRange(self::optionalDouble($name, $default), $min, $max);
    }
    ...
    public static function optionalIsArray($name)
    {
        $optional = self::arguments()->optional($name);
        if (Predicates::isNotEmpty($optional)){
            Preconditions::checkArray($optional);
        }
        return $optional;
    }
    ...
    {
        throw new ProtocolException(self::STATUS_TEMPORARY_REDIRECT, $notice, $message, $payload);
    }

    public static function badRequest($payload = NULL, $notice = NULL, $message = NULL, $hint = NULL)
    {
        throw new ProtocolException(self::STATUS_BAD_REQUEST, $notice, $message, $payload, $hint);
    }

    public static function unauthorized($payload = NULL, $notice = NULL, $message = NULL)
    {
        throw new ProtocolException(self::STATUS_UNAUTHORIZED, $notice, $message, $payload);
    }

    public static function forbidden($payload = NULL, $notice = NULL, $message = NULL)
    {
        throw new ProtocolException(self::STATUS_FORBIDDEN, $notice, $message, $payload);
    }

    public static function notFound($payload = NULL, $notice = NULL, $message = NULL)
    {
        throw new ProtocolException(self::STATUS_NOT_FOUND, $notice, $message, $payload);
    }

    public static function requestTimeout($payload = NULL, $notice = '', $message = NULL)
    {
        throw new ProtocolException(self::STATUS_REQUEST_TIMEOUT, $notice, $message, $payload);
    }

    public static function upgradeRequired($payload = NULL, $notice = NULL, $message = NULL)
    {
        throw new ProtocolException(self::STATUS_UPGRADE_REQUIRED, $notice, $message, $payload);
    }

    public static function tooManyRequest($payload = NULL, $notice = NULL, $message = NULL)
    {
        throw new ProtocolException(self::STATUS_TOO_MANY_REQUEST, $notice, $message, $payload);
    }

    public static function internalError($payload = NULL, $notice = NULL, $message = NULL)
    {
        throw new ProtocolException(self::STATUS_INTERNAL_ERROR, $notice, $message, $payload);
    }
    ...
    public static function stringify($array) {
        if (Predicates::isNotArray($array)) {
            return $array;
        }
        foreach ($array as $key => $value) {
            if (Predicates::isNotNull($value) && Predicates::isNotString($value)) {
                if (Predicates::isObject($value)) {
                    $value = get_object_vars($value);
                }
                if (Predicates::isArray($value)) {
                    $value = self::stringify($value);
                } else if (Predicates::isNumeric($value)) {
                    $value = strval($value);
                }
                $array[$key] = $value;
            }
    ...
    public static function toJson($arg)
    {
        return json_encode(self::stringify($arg));
    }
}
?>
