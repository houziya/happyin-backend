...
class CUserGroup{
    ...
    public static function getAllGroup() 
    {
        ...
        if (CUserSession::getUserType() == 1) {
            $list = $db -> select($columns.',u.user_name as owner_name')
                ->from(self::getTableName()." g") 
                ->leftjoin(CUser::getTableName()." u","g.group_id = u.user_group")
                ->where(['u.parent_user_id' => CUserSession::getUserId()])
                ->andWhere('u.user_id !=' . CUserSession::getUserId())
                ->all();
        } else {
