...
class ChannelController extends Controller_Abstract {
    ...
    private static $spreadMainChannel = HI\TableName\SPREAD_MAIN_CHANNEL;
    
    private static $spreadSubChannel = HI\TableName\SPREAD_SUB_CHANNEL;
    
    private static $spreadChannelStat = HI\TableName\SPREAD_CHANNEL_STAT;
...
//                     $fmp=$channel['platform'];
    ...
    */
    public function channelAction() {
        ...
        ->from(HI\TableName\SPREAD_CHANNEL_STAT." as s")->leftJoin(HI\TableName\SPREAD_SUB_CHANNEL." as c", "s.sid = c.id")
        ->leftJoin(HI\TableName\SPREAD_MAIN_CHANNEL." as m", "m.cid = c.cid")
        ...
        if (Protocol::optional('method', '') == 'link' && Protocol::getMethod() == "GET") {
            ...
            /*分渠道商 主渠道商关联查询  */
                ...
                foreach ($subchannellist as $key => $val) {
                    ...
                    $subchannellist[$key]['link'] = HI\APP_URL . "/Us/Stat/adClick?d=" . $val['channel_code'] . "&t=" . $val['channel_token'];
                }
                if ($subchannellist) {
    ...
    public function subSponsorsAddressAction()
    {
        ...
        if(Protocol::getMethod() == "GET") {
            ...
            $this->getView()->assign('subLink', HI\APP_URL . "Catalog/Stat/adClick?d=" . $sub['channel_code'] . "&t=" . $sub['channel_token']);
