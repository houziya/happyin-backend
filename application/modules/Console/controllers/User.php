...
class UserController extends Controller_Abstract
{
    public function IndexAction()
    {
        ...
        $orderInfo = CStat::queryOrderInfos($user_info['coupon_code']);
        $selfSaleInfo = CStat::doTheirAgentsInfo($user_info['user_id']);
        $saleDetailInfo = CStat::doTheirAgentsInfo($user_info['user_id'], 1);
        ...
        $this->getView()->assign('order', $orderInfo);
        $this->getView()->assign('sale', $selfSaleInfo);
        $this->getView()->assign('detail', $saleDetailInfo);
        $this->display('index');
    }
...
//                 }else{
    ...
    public function AddAction()
    {
        ...
        if (Protocol::getMethod() == "POST") {
            ...
            try{
                ...
                if($exist){
                ...
                } else if($data->required('password')=="" || $data->required('real_name')=="" || $data->required('mobile') =="" || $data->required('email') =="" ){
                    throw new InvalidArgumentException(Console\ADMIN\NAME_CONFLICT);
                } else {
                    if ($data->required('user_type') == 1) {
                        $db = new \yii\db\Query;
                        $top = $data->optional('topic', '') + 1;
                        $sql = "select group_id from c_user_group  where group_name like '{$top}%' ";
                        $list = $db -> createCommand() -> setSql($sql) -> queryOne();
                    } else {
                        $list = '';
                    }
                    $input_data = array ('user_name' => $data->required('user_name'), 'password' => md5 ( $data->required('password') ),
                        'real_name' => $data->required('real_name'), 'mobile' => $data->required('mobile'), 'email' => $data->required('email'),
                        'user_desc' => $data->optional('user_desc'), 'user_group' => empty($list) ? $data->optional('user_group') : $list['group_id'], 'alipay' => $data->optional('alipay', ''),
                        'coupon_code' => $data->optional('coupon', ''), 'user_type' => $data->required('user_type'), 'parent_user_id' => $current_user_info['user_id'],
                        'topic' => $data->optional('topic', '')
                    );
                    /* 上传标识图 */
                    if ($data->required('user_type') == 1) {
                        if (!empty($_FILES['coupon_image'])) {
                            try{
                                $input_data['coupon_image'] = CUser::replacementProcess($_FILES['coupon_image'], 8); //得到更换封面图的处理结果
                    ...
                    if ($data->required('user_type') == 1) {
                        /* 顶级代理 */
                        if ($data->optional('topic', '') == 1) {
                            CUser::updateUser($user_id, ['parent_user_id' => $user_id]);
                        }
                    }
                    if ($user_id) {
                        $input_data['password']="";
        ...
        $this->getView()->assign('userInfo', $current_user_info);
        $this->display('add');
    }
    public function UsersAction()
    {
        ...
        if($data->optional('search')){
            ...
            $user_infos=CUser::search($current_user_info['user_id'], $data->optional('user_group'),$data->optional('user_name'),$start , $page_size);
        }else{
            if($current_user_info['user_id'] !=1){
            ...
            if (CUserSession::getUserType() == 1) {
                $user_infos=CUser::getAllUsers($start,$page_size, 1); //微商渠道
            } else {
                $user_infos=CUser::getAllUsers($start,$page_size);
            }
        }
        $page_html=CPagination::showPager("users?user_group=".$data->optional('user_group')."&user_name=".$data->optional('user_name')."&search=".$data->optional('search'),$page_no,$page_size,$row_count);
        //追加操作的确认层
    ...
    public function ModifyAction()
    {
        ...
        if (Protocol::getMethod() == "POST") {
            $transaction = Yii::$app->db->beginTransaction();
            try {
                if($data->optional('real_name') == "" || $data -> optional('mobile') == "" || $data ->optional('email') == "" ) {
                    throw new InvalidArgumentException(Console\ADMIN\NEED_PARAM);
                } else {
                    $update_data = ['real_name' => $data->required('real_name'), 'mobile' => $data->required('mobile'),
                            'email' => $data->required('email'), 'user_desc' => $data->optional('user_desc'),
                        ];
                    if ($current_user_info['user_group'] == HI\Config\Console\ADMIN_ID) {
                        if (!empty($data->optional('coupon_code')) ) {
                            $update_data['coupon_code'] = $data->optional('coupon_code');
                        }
                        if (!empty($data->optional('alipay'))) {
                            $update_data['alipay'] = $data->optional('alipay');
                        }
                        /* 微商账号上传标识图 */
                        if ($user['user_type'] == 1) {
                            if (!empty($_FILES['coupon_image'])) {
                                try{
                                    $update_data['coupon_image'] = CUser::replacementProcess($_FILES['coupon_image'], 8); //得到更换封面图的处理结果
                                } catch (InvalidArgumentException $e) {
                                    CAdmin::alert($this, "error", $e->getMessage());
                                }
                            }
        ...
        $this->getView()->assign('group_options',$group_options);
        $this->getView()->assign('current_user_info', CUserSession::getSessionInfo());
        $this->display('modify');
    }
...
//         $user_name = $password = $real_name = $mobile = $email = $user_desc = $change_password = $show_quicknote = $old = $new= '';
    ...
    public function ajaxAction()
    {
        $data=Protocol::arguments();
        if (Protocol::getMethod() == "POST") {
            if ($data->required('content') == 1) { //一级代理
                echo json_encode(['top_agent']);
            } else {
                $query = (new yii\db\Query())->select('user_id, user_type, user_name')
                    ->from('c_user')
                    ->where(['user_type' => ($data->required('content') - 1)])
                    ->all();
                 if (empty($query)) {
