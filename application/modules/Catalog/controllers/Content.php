...
class ContentController extends Controller_Abstract
{
    ...
    private static function parseFileName($fileName, $objectIdGenerator)
    {
        ...
        $tmp->filter = NULL;
        $tmp->platform = NULL;
        return array_reduce($spec, function($carry, $item) {
                $array = array_slice($item, 1);
                $key = $item[0];
                Preconditions::checkNull($carry->$key);
                $carry->$key = count($array) > 1 ? $array : $array[0];
                return $carry;
            }, $tmp);
        }
    ...
    private static function validateSpec($fileName, $spec, $isLive = false)
    {
        try {
            ...
            if (Predicates::isNotNull($spec->resolution)) {
                $limit = json_decode(HI\Config\CONTENT_GENERATOR_RESOLUTION_LIMIT);
            ...
            if (Predicates::isNotNull($spec->quality) && !in_array($spec->quality, json_decode(HI\Config\CONTENT_GENERATOR_QUALITY))) {
    ...
    {
        return Protocol::userAgent() === HI\Config\QCloud\COS_USER_AGENT;
    }
    ...
    private static function checkLimit($fileName)
    {
        ...
        $key = HI\Config\CONTENT_GENERATOR_RATE_LIMIT_KEY_PREFIX . $fileName;
        $generation = Yii::$app->redis->incr($key);
        /* workaround crs expire bug */
        Yii::$app->redis->expire($key, HI\Config\CONTENT_GENERATOR_RATE_LIMIT_EXPIRE);
        if ($generation > HI\Config\CONTENT_GENERATOR_RATE_LIMIT) {
    ...
    {
        try {
            ini_set('memory_limit', HI\Config\CONTENT_GENERATOR_MEMORY_LIMIT);
    ...
    {
        $this->doGenerate("/admin/images/", function($fileName) { return NULL; });
    }
