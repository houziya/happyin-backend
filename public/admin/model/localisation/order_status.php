...
class ModelLocalisationOrderStatus extends Model {
	...
	public function getOrderStatuses($data = array()) {
	    $whereStatus = " and order_status_id in ('1','2','5','14')";
	    $whereStatus = "";
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "order_status WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'".$whereStatus;
			...
			if (!$order_status_data) {
				$query = $this->db->query("SELECT order_status_id, name FROM " . DB_PREFIX . "order_status WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'" .$whereStatus. " ORDER BY name");
