...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET 
	        ...
	        quantity_zj = '" . (int)$data['quantity_zj'] . "', 
		    standard_explain = '" . $this->db->escape($data['standard_explain']) . "', 
		    quantity_sd = '" . (int)$data['quantity_sd'] . "', 
	...
	public function editProduct($product_id, $data) {
		...
		//2016/4/12添加父商品
		if (!empty($data['product_parent'])) {
		    //$this->db->query("DELETE FROM " . DB_PREFIX . "extend_to_product WHERE product_id = '" . (int)$product_id . "'");
		    foreach ($data['product_parent'] as $parentPid) {
		        $this->db->query("UPDATE " . DB_PREFIX . "product SET isbn = '" . (int)$parentPid . "' where product_id = '" . (int)$product_id ."'");
		    }
		}else {
		    $this->db->query("UPDATE " . DB_PREFIX . "product SET isbn = '' where product_id = '" . (int)$product_id ."'");
		}
	...
	public function getProduct($product_id, $type = NULL) {
	    if ($type) {
	...
	public function getProductsByProductIds($product_ids, $fields = "") {
	    ...
	    //$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p WHERE p.product_id in (" . $product_ids . ")");
	    return $query->rows;
	}
	public function getProducts($data = array(), $type = NULL)
	{
	    if ($type) {
	       $sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' and p.isbn=''";
	    } else {
	...
	public function getWarningProductNum()
	{
