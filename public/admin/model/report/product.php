...
class ModelReportProduct extends Model {
	...
	public function getStockWarningTotal()
	{
	    $sql = "SELECT COUNT('pd.product_id') as total FROM `" . DB_PREFIX . "product`as p inner join `" . DB_PREFIX . "product_description`as pd on p.product_id = pd.product_id WHERE p.quantity < (SELECT VALUE FROM `".DB_PREFIX."setting` WHERE CODE='config' AND setting.key='config_stock_warning' AND VALUE !=0) and pd.language_id = 2 ";
	    $query = $this->db->query($sql);
        return $query->row['total'];
	}
	
	public function getStockWarningValue()
	{
	    $query = $this->db->query("SELECT VALUE as v FROM `".DB_PREFIX."setting` WHERE CODE='config' AND setting.key='config_stock_warning' AND VALUE !=0");
	    return $query->row['v'];
	}
}