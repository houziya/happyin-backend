...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
			...
			if(isset($this->session->data['splitting_code']) && in_array($this->session->data['splitting_code'], [0, 1])) {
			    // Complete Orders
			    $data['complete_status_total'] = $this->model_sale_order->getHzTotalOrders(array('filter_order_status' => implode(',', Order::$configCompleteStatus)));
			    $data['complete_status'] = $this->url->link($_REQUEST['route'], 'token=' . $this->session->data['token'] . '&filter_order_status=' . implode(',', Order::$configCompleteStatus), 'SSL');
			    // Processing Orders
			    $data['processing_status_total'] = $this->model_sale_order->getHzTotalOrders(array('filter_order_status' => implode(',', Order::$configHzProcessingStatus)));
			    $data['processing_status'] = $this->url->link($_REQUEST['route'], 'token=' . $this->session->data['token'] . '&filter_order_status=' . implode(',', Order::$configHzProcessingStatus), 'SSL');
			}else {
			    $data['complete_status_total'] = $this->model_sale_order->getTotalOrders(array('filter_order_status' => implode(',', Order::$configCompleteStatus)));
			    $data['complete_status'] = $this->url->link($_REQUEST['route'], 'token=' . $this->session->data['token'] . '&filter_order_status=' . implode(',', Order::$configCompleteStatus), 'SSL');
			    // Processing Orders
			    $data['processing_status_total'] = $this->model_sale_order->getTotalOrders(array('filter_order_status' => implode(',', Order::$configProcessingStatus)));
			    $data['processing_status'] = $this->url->link($_REQUEST['route'], 'token=' . $this->session->data['token'] . '&filter_order_status=' . implode(',', Order::$configProcessingStatus), 'SSL');
			}			
