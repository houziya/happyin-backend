<?php
include_once APP_PATH . '/application/library/CosFile.php';
...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		$data['images'] = [];
		$query = $this->db->query("SELECT upload_id, `filename`, concat('" . HI\Config\QCloud\IMAGE\DOMAIN . "/admin/images/',`filename`,'.jpg') as url, name FROM `upload`");
		
		if ($query->rows) {
		    foreach ($query->rows as $imageSub) {
		        $data['images'][] = array(
		            'upload_id'  => $imageSub['upload_id'],
		            'thumb' => $imageSub['url'],
		            'name'  => $imageSub['name'],
		            'filename'  => $imageSub['filename'],
		            'type'  => 'image',
		            'path'  => $imageSub['url'],
		            'href'  => $imageSub['url']
		        );
	...
	public function upload() {
		...
		if (!$json) {
			...
			$uploadReturn = CosFile::uploadFile($this->request->files['file'], 1, 8);
			if(!empty($uploadReturn)) {
			    $this->db->query("INSERT INTO `upload` SET `filename` = '" . $uploadReturn['subUrlName'] . "', `date_added` = '" . date('Y-m-d H:i:s', time()) . "'");
			}
		...
		if (!$json) {
			...
			if(!empty($paths)) {
			    ...
			    foreach ($paths as $subPath) {
			         $i++;
			         if($i === count($paths)) {
			             $uploadFileName .="'".$subPath."'";
			         }else {
			             $uploadFileName .="'".$subPath."',";
			         }
			    }
			    $this->db->query("DELETE FROM `upload` WHERE filename in (" . $uploadFileName . ")");
			}
