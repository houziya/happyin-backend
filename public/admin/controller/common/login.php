...
class ControllerCommonLogin extends Controller {
	...
	public function index() {
		...
		if ($this->user->isLogged() && isset($this->request->get['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
		    self::checkSplittingCode();
		    if(isset($this->session->data['splitting_code'])) {
		        if(in_array($this->session->data['splitting_code'], [0, 1])) {
	...
	private function checkSplittingCode() {
	    ...
	    switch ($user_group['name']) {
	        case "hangzhou":
	            ...
	            $this->session->data['splitting_code'] = $filter_splitting_code;
	            break;
	        case "shandong":
