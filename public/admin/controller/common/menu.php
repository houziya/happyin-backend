...
class ControllerCommonMenu extends Controller {
	public function index() {
		...
		if(isset($this->session->data['splitting_code'])) {
		    $data['text_hz_order'] = ($this->session->data['splitting_code'] == 0) ? "山东" . $this->language->get('text_hz_order') : "杭州" . $this->language->get('text_hz_order');
		}
		$data['text_processing'] = '打单发货';
		$data['text_statistic_sale'] = '营收';
		$data['text_statistic_channel'] = '渠道';
		$data['text_statistic_cost_accounting'] = '成本核算';
		$data['text_parts_management'] = '配件列表';
		$data['text_parts'] = '配件管理';
		$data['text_statistic_coupon'] = '优惠劵';
		$data['text_order_status'] = $this->language->get('text_order_status');
		$data['text_opencart'] = $this->language->get('text_opencart');
		$data['text_payment'] = $this->language->get('text_payment');
		$data['text_product'] = $this->language->get('text_product');
		$data['text_print'] = '冲印类型';
		...
		$data['text_openbay_links'] = $this->language->get('text_op enbay_links');
		...
		$data['text_cost_price'] = $this->language->get('text_cost_price');
		$data['text_cost_accounting'] = $this->language->get('text_cost_accounting');
		$data['text_cost_accounting_sd'] = '山东成本';
		$data['text_cost_accounting_zj'] = '浙江成本';
		...
		if (in_array($user_group['name'], ['hangzhou', 'shandong'])) {
		    $data['hz_order'] = $this->url->link('sale/hz_order', 'token=' . $this->session->data['token'], 'SSL');
		    $data['hz_processing'] = $this->url->link('sale/hz_order/processing', 'token=' . $this->session->data['token'], 'SSL');
		}else {
		    $data['hz_order'] = $this->url->link('sale/hz_order', 'token=' . $this->session->data['token'] . "&splitting_code=1", 'SSL');
		    $data['sd_order'] = $this->url->link('sale/hz_order', 'token=' . $this->session->data['token'] . "&splitting_code=0", 'SSL');
		    $data['hz_order_processing'] = $this->url->link('sale/hz_order/processing', 'token=' . $this->session->data['token'] . "&splitting_code=1", 'SSL');
		    $data['sd_order_processing'] = $this->url->link('sale/hz_order/processing', 'token=' . $this->session->data['token'] . "&splitting_code=0", 'SSL');
		    $data['parts_management'] = $this->url->link('parts/parts/index', 'token=' . $this->session->data['token'], 'SSL');
		    
		    $data['statistic_sale'] = $this->url->link('statistic/statistic/sale', 'token=' . $this->session->data['token'], 'SSL');
		    $data['statistic_channel'] = $this->url->link('statistic/statistic/channel', 'token=' . $this->session->data['token'], 'SSL');
		    $data['statistic_cost_accounting'] = $this->url->link('statistic/statistic/accounting', 'token=' . $this->session->data['token'], 'SSL');
		    $data['statistic_coupon'] = $this->url->link('statistic/statistic/coupon', 'token=' . $this->session->data['token'], 'SSL');
		    
    		$data['analytics'] = $this->url->link('extension/analytics', 'token=' . $this->session->data['token'], 'SSL');
    		$data['print'] = $this->url->link('catalog/print', 'token=' . $this->session->data['token'], 'SSL');
    		...
    		$data['cost_price'] = $this->url->link('report/price', 'token=' . $this->session->data['token'], 'SSL');
    		$data['cost_accounting'] = $this->url->link('report/accounting', 'token=' . $this->session->data['token'], 'SSL');
    		$data['cost_accounting_sd'] = $this->url->link('report/accounting_sd', 'token=' . $this->session->data['token'], 'SSL');
    		$data['cost_accounting_zj'] = $this->url->link('report/accounting_zj', 'token=' . $this->session->data['token'], 'SSL');
    		...
    		$data['shipment'] = $this->url->link('express/shipment', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['user_group_name'] = $user_group['name'];
