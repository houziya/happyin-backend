<?php
class ControllerReportAccountingZj extends Controller {
	private $error = array();
	
	public function index()
	{
	    $this->language->load('report/accounting_zj');
	    $this->document->setTitle($this->language->get('heading_title'));
	    $this->load->model('report/accounting_zj');
	    $this->getList();
	}
	...
	protected function getList()
	{
	    ...
	    $data['breadcrumbs'][] = array(
	            'text' => '浙江成本核算',
	            'href' => $this->url->link('report/accounting_zj', 'token=' . $this->session->data['token'] . $url, 'SSL')
	    );
...
// 	    $data['pagination'] = $pagination->render();
	    $this->response->setOutput($this->load->view('report/accounting_zj_list.tpl', $data));
	}
	    ...
	    foreach ($orders as $date => $order) {
	        foreach ($order as $order_products) {
	            foreach ($order_products['product'] as $order_product) {
	                if ($order_products['origin'] == 1365) {
	                    if (!in_array($order_product['product_id'], [53,54,81])) {
	                        $price = 8;
	                    } else {
	                        $price = 6;
	                    }
	                } else {
	                    $price = $order_products['shipping'];
	                }
	            }
	            $origin_cost[$order_products['origin']] = @$origin_cost[$order_products['origin']] + $price;
	        }
