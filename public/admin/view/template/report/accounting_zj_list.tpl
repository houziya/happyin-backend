      ...
      <ul class="breadcrumb">
					...
					<form method="post" enctype="multipart/form-data" id="form-product">
						<div class="table-responsive">
							<table class="table table-bordered table-hover">
								...
								<tr>
									<td class="text-center" rowspan="2">日期</td>
									<td class="text-center" rowspan="2">订单数</td>
									<td class="text-center" rowspan="2">运费</td>
									<td class="text-center" colspan="2">5英寸商品</td>
									<td class="text-center" colspan="2">6英寸商品</td>
									<td class="text-center" colspan="2">LOMO卡</td>
									<td class="text-center" colspan="2">照片卡</td>
									<td class="text-center" colspan="2">方格1x1(黑色边框)</td>
									<td class="text-center" colspan="2">方格1x1(白色边框)</td>
									<td class="text-center" colspan="2">方格2x2(黑色边框)</td>
									<td class="text-center" colspan="2">方格2x2(白色边框)</td>
									<td class="text-center" colspan="2">方格3x3(黑色边框)</td>
									<td class="text-center" colspan="2">方格3x3(白色边框)</td>
									<td class="text-center" colspan="2">方格4x4(黑色边框)</td>
									<td class="text-center" colspan="2">方格4x4(白色边框)</td>
									<td class="text-center" colspan="2">大海报</td>
									<td class="text-center" colspan="2">记忆盒子</td>
									<td class="text-center" colspan="2">相册2043</td>
									<td class="text-center" colspan="2">相册2045</td>
									<td class="text-center" colspan="2">相册2046</td>
									<td class="text-center" colspan="2">画册</td>
									<td class="text-center" rowspan="2">成本合计</td>
								</tr>
								<tr>
									<!--五英寸-->
									...
									<td class="text-center">成本</td>
									<!--LOMO卡-->
									<td class="text-center">套数/25</td>
									<td class="text-center">成本</td>
									<!--照片卡-->
									<td class="text-center">套数/15</td>
									<td class="text-center">成本</td>
									<!--方格1x1黑-->
									<td class="text-center">冲印数量</td>
									<td class="text-center">成本</td>
		...
		if(thisHost.indexOf('dev') == 0) {
			...
			dataHZPathArr = [
				...
				'products_cost.55.933.cost',
				//照片卡 61
				'products_cost.61.933.number',
				'products_cost.61.933.cost',
				//1x1黑 68
				'products_cost.68.933.number',
				'products_cost.68.933.cost',
				//1x1白 63
			...
			$('#search-by-ajax').on('click',function(){
				...
				buildExcel(searchMonth,dataHZPathArr);
			});
				...
				$.ajax({
					...
					success: function(d){
						...
						for(var i=0; i<dateArr.length; i++) {	//循环时间（纵向表头）
							...
							for(var j=0; j<dataPathArr.length; j++) {	//循环配置（横向表头）
								...
								for(var k=0; k<dataPathSplitWithSignAndArr.length; k++) {	//如果单元格是多个数据求和，循环该配置
									...
									for(var l=0; l<dataPathSplitWithSignDotArr.length; l++) {	//循环配置内字符，补全地址
										...
										if(dataPathSplitWithSignDotArr[l] == 'cost'){
											try{
												var data3 = eval(data1);
												if(!data3){
													data3 = 0;
												}
												dataTotalCost += data3;
							...
							appendChild(dateArr[i],dataTotalCost);
						}


						for (var m=0; m<dataPathArr.length+1; m++) {
							var sum = 0;
			...
			$(function(){
				buildExcel(jsGetMonth(),dataHZPathArr);
			})
